...
public class ExcelParseUtil {
	 ...
	 * @throws Exception
	 */
	public static synchronized List<List<String>> getCellContentsBySheetName(
		...
		try {
			...
			throw new Exception(filePathName + "文件不存在", e);
		} catch (Exception e) {
			e.printStackTrace();
			throw new Exception("解析excel数据出错", e);
		}
	}
...
//				value = "ERROR_" + String.valueOf(cell.getErrorCellValue());
	...
	public static synchronized String[][] getExcelDataBySheetName(
		...
		try {
			...
			e.printStackTrace();
			throw new Exception("处理excel数据出错", e);
		}
	}
	...
	public static int getSheetIndex(String sheetName, String filePathName) throws Exception {
		try {
			...
			throw new Exception("没有找到Excel文件：" + filePathName, e);
		} catch (IOException e) {
			throw e;
		}
	}
